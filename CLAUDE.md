...
- `article-backlog.md` — the stock of planned articles, per-series plans, and publishing-order dependencies. Open it when choosing what to write next or sequencing posts. Its section 0 defines the article ID (`📝PR3-01`: category letters, series number, item number). Refer to articles by ID, never by title or by position in the publishing order.
...
- `backlog_id` is the article ID from `docs/planning/article-backlog.md` section 0, without the 📝. Chirpy ignores it; it exists so a post file can be matched to its backlog entry. `ニュース` and `Tips・小技` posts get theirs (`NW12-NN` / `TP-NN`) when published. Posts published before 2026-09-25 have none yet; add it the next time their body is edited; `お知らせ` posts have none.
...
- 投稿ファイル名が決まる前は `verification/_pending-<ID>-<英語スラッグ>/` に置き（例: `_pending-GK6-00-inventory-your-work-for-ai`。IDは `article-backlog.md` 0章）、ファイル名が決まったら改名する
...
| `site/_plugins/posts-lastmod-hook.rb` | Sets `last_modified_at` from git history, so edited posts show an updated date. Only a last commit dated after the publish day (taken from the filename) counts — Chirpy shows 「更新」 whenever `last_modified_at` differs from `date`, so pre-publish and same-day edits would otherwise read as updates. Needs the full history — `.github/workflows/pages.yml` checks out with `fetch-depth: 0` for this reason; with the default shallow clone the hook silently never fires |
...
`docs/` has two subfolders: `docs/planning/` holds the three planning docs described at the top, and `docs/reference/` holds reference material about how the blog itself works. Currently that is `docs/reference/blog-mechanism.html`, a self-contained one-page explainer (publishing flow, Jekyll, folder layout, the Claude Code helpers) written for a middle-school reader; open it directly in a browser. It loads web fonts from Google Fonts, so it needs a network connection to render in the intended typeface. When the setup changes (build flow, published folders, skills/agents), update this page and the slide decks below along with the sections above. `docs/reference/blog-mechanism-slides/` holds the source of a 23-slide presentation version of the same story (plus a section on how generative AI works, and a closing QR code to the blog). `docs/reference/blog-mechanism-slides-ai-focus/` is a separate 40-slide version for an AI-interested audience: five parts (blog / how AI works / skills / sub-agents / division of labor between people and AI), a divider slide before each part, and much more detail on the skills and agents under `.claude/`. When skills or agents are added, removed or renamed, update the second deck too. Each folder has its own `README.md` for how to rebuild and re-upload it.
