...
add("helpers",
    ...
    + row(
        ...
        + col(
            ...
            + li("post-reviewer", "書き上がった下書きを、ルールブックに照らして点検")
...
add("chain",
    ...
    + col(
        ...
        + row(cstep("4", "点検する", "post-reviewer") + cstep("5", "図解をつける", "/add-infographic") + cstep("6", "送る（push）", "人がやる", True), gap=24),
        gap=24)
    + p("黄色が、人にしかできない部分。それ以外は、AIが手伝う。", 32, MUTED),
    "記事ができるまでは6ステップです。1つ目のネタ出しは、AIが切り口をいくつも出してくれます。2つ目が大事で、私が自分の体験を話します。3つ目でAIが下書きを作り、4つ目で別のAIがルールブックに照らして点検し、5つ目で図解を作ります。そして6つ目、pushして公開するのは人です。黄色い2つが人の仕事です。")
