...
add("agent-file",
    ...
    + row(
        ...
        + col(
            card(h3("name と description", 34, BLUE_T) + p("呼ぶときの名前と、「いつ頼むか」の説明。メインのAIはこれを見て、頼む相手を選ぶ。", 28, INK, lh=1.5), pad="22px 32px 26px", gap=6, extra="flex:1; justify-content:center")
            ...
            + card(h3("本文：役割と、返すもの", 34, BLUE_T) + p("「あなたは校閲担当です」と役割を決め、何を返すかを書く。", 28, INK, lh=1.5), pad="22px 32px 26px", gap=6, extra="flex:1; justify-content:center"),
            gap=16, extra="flex:1"),
        gap=40, extra="flex:1"),
    "サブエージェントの実物も、スキルと同じく1つの文書ファイルです。これは校閲担当、post-reviewerの冒頭です。名前と、いつ頼むかの説明。そして黄色い行のtools、これが持ち物リストです。ファイルを読む、探す、コマンドを実行する道具は渡してありますが、文章を書き換える道具は持たせていません。本文は『あなたはこのブログの校閲担当です』で始まり、下書きのパスを受け取って、照合結果だけを返す、ファイルは編集しない、と書いてあります。校閲担当は指摘しかできない作りになっています。")
...
add("agents-three",
    ...
    + row(
        ag3("idea-generator", "ネタ出し担当", "思いつきや外部の記事から、記事ネタの切り口を3〜5案出す。",
            "案ごとに「何で勝負するか」を名指しする。書き手が話していない体験は、でっち上げない。")
        + ag3("idea-reviewer", "ネタ帳の門番", "ネタ帳に積んでよいか審査する。重複・分類・価値、ChatGPTで代わりが利かないかを見る。",
              "落とすときは、素材が活きる別の置き場所を必ず示す。")
        + ag3("post-reviewer", "校閲担当", "書き上がった下書きを、書き方のルールに照らして点検する。",
              "直す価値があるものだけを、直し文つきで返す。粗探しで件数を稼がない。"),
        gap=24, extra="flex:1"),
    "このブログで働くサブエージェントは3人います。1人目はネタ出し担当のidea-generator。思いつきや外部の記事から、記事ネタの切り口を3〜5案出します。守っているのは、案ごとに何で勝負するかを名指しすること、そして書き手が話していない体験をでっち上げないことです。2人目はネタ帳の門番、idea-reviewer。ネタ帳に積んでよいかを審査します。既存のネタと重なっていないか、価値があるか、ChatGPTで代わりが利かないか。落とすときは、その素材が活きる別の置き場所を必ず示す決まりです。3人目が校閲担当のpost-reviewerで、書き上がった下書きを点検します。直す価値があるものだけを、直し文つきで返します。")
...
add("post-reviewer",
    head("PART 4 ／ サブエージェント", "校閲担当は、6つの観点で下書きを点検する")
    + col(
...
add("teamwork",
    ...
    + col(
        lane("ネタを積む", [
            ...
            chip("idea-generator", "切り口を3〜5案返す", "a", True),
            chip("人が選ぶ", "1本に絞る", "h"),
            chip("idea-reviewer", "重複と価値を審査する", "a", True),
            chip("ネタ帳に追加", "公開順への影響も確認", "o")])
        + lane("記事を書く", [
            chip("/write-post", "テーマを決めて、質問する", "s", True),
            chip("人が話す", "失敗談も含めて", "h"),
            chip("/write-post", "話した言葉を活かして下書き", "s", True),
            chip("post-reviewer", "6つの観点で点検する", "a", True),
    ...
    "この2つを、実際にどうつないでいるかを図にしました。上の段がネタを積むとき。add-ideaのスキルが起点を見分けて素材を掘り、ネタ出し担当が切り口を3〜5案返します。ここで人が1本に選びます。そのあと門番のidea-reviewerが審査して、ネタ帳に追加します。下の段が記事を書くとき。write-postのスキルがテーマを決めて質問し、私が体験を話し、それを活かして下書きを書き、校閲担当が点検して、最後は人が直します。黄色が人、青がスキル、白が専門担当のサブエージェントです。人が入る場所は、選ぶ、話す、直す。つまり判断が必要なところです。")
