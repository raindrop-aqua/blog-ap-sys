...
add("write-post-flow",
    ...
    + col(
        ...
        + row(step("5", "下書き", "聞いた体験をもとに書く")
              + step("6", "校閲", "専門担当のアカネに点検してもらう", "agent")
...
# ---- ひみつ13 agents-three
def agent_card(icon, name, nick, role, body, tools, used):
    return card(
        ...
        + row(h3(nick, 44) + p(role, 28, MUTED, 700, lh=1.3), gap=16, extra="align-items:baseline")
        + p(body, 28, MUTED, lh=1.55)
...
add("agents-three",
    ...
    + row(
        agent_card("Search", "idea-generator", "💡 ヒラク", "ネタ出し担当", "思いつきから、記事ネタの切り口を複数ならべる。選ぶのは人。", "道具：読む・探す・ネットで調べる", "呼ぶ場面：/add-idea の中")
        + agent_card("Lock", "idea-reviewer", "🗃️ クラ", "ネタ帳の門番", "重複していないか、書けるネタか、価値があるかを審査する。落とすときは代わりの置き場所も示す。", "道具：読む・探す", "呼ぶ場面：/add-idea の中")
        + agent_card("CheckCircle", "post-reviewer", "🖍️ アカネ", "校閲担当", "下書きをルールブックと照らして点検する。直す価値のある指摘だけを、直し方つきで返す。", "道具：読む・探す", "呼ぶ場面：/write-post の校閲"),
        gap=24, extra="flex:1"),
    "このブログの専門担当は3人で、それぞれ名前があります。1人目、ネタ出し担当のヒラク。切り口を拓くからヒラクです。思いつきから、記事ネタの切り口を複数並べてくれます。どれにするかを選ぶのは人です。2人目、クラ。ネタ帳という蔵の番人で、足してよいかを審査します。、重複や、書けないネタを見つけます。落とすときも、代わりの置き場所を示します。3人目、アカネ。原稿に赤を入れる校閲担当で、書き上がった下書きをルールブックと照らして点検します。3人とも、点検して指摘するだけで、ファイルは書き換えません。直すのは、人とメインのAIの仕事です。")
...
add("teamwork",
    ...
    "ひみつ14、最後のひみつです。ここまでの部品を、全部つなげてみます。上の段は、ネタをためる流れ。人が思いつきを話すと、add-ideaという手順書が話を聞き、ネタ出し担当のヒラクが切り口を並べます。ここで、どれで書くかを選ぶのは人です。次に門番のクラが審査して、OKならネタ帳に追加されます。下の段は、記事にする流れ。write-postを始めて、人が体験を話し、AIが下書きを書き、校閲担当のアカネが点検し、人が直して送ります。送ったあとは、最初に見たロボットが公開してくれます。どの担当も、動く前にルールブックを読みます。そして、最後に決めるのは、いつも人です。",
    gap=26)
